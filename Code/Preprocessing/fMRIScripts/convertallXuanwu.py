...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        if ('t1' in s.protocol_name) or ('T1' in s.protocol_name):
            info[t1w].append(s.series_id)
        if ('TSE' in s.protocol_name) or ('T2' in s.protocol_name) or ('t2' in s.protocol_name):
            info[tse].append(s.series_id)
        if ('DTI' in s.protocol_name) and (s.dim3 == 74) and (s.dim4 == 32) or ('dwi' in s.protocol_name):
        ...
        if ('restingstate' == s.protocol_name) or ('resting' in s.protocol_name) or ('BOLD' in s.protocol_name) or ('bold' in s.protocol_name) :
